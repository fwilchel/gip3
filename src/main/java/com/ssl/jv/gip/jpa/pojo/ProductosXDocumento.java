...
@NamedQueries({
	...
	@NamedQuery(name = ProductosXDocumento.FIND_BY_DOCUMENTO_COLECCIONES, query = "SELECT p FROM ProductosXDocumento p JOIN FETCH p.productosInventario pi JOIN FETCH pi.productosInventarioComext pic JOIN FETCH pic.tipoLoteoic tl   WHERE p.id.idDocumento = :idDocumento"),
