...
@NamedQueries(value = {
  ...
  @NamedQuery(name = ProductosXCliente.NQ_BUSCAR_PRODUCTOS_X_CLIENTES_ACTIVOS, query = "SELECT pxc FROM ProductosXCliente pxc JOIN FETCH pxc.cliente JOIN FETCH pxc.moneda JOIN FETCH pxc.productosInventario JOIN FETCH pxc.puntoVenta WHERE pxc.cliente.id = :idCliente AND pxc.activo = true ORDER BY pxc.productosInventario.sku")
})
public class ProductosXCliente implements Serializable {
  ...
  public static final String NQ_BUSCAR_PRODUCTOS_X_CLIENTES_ACTIVOS = "ProductosXCliente.buscarProductosXClienteActivos";
