...
public class ItemCostoLogisticoDAO extends GenericDAO<ItemCostoLogistico> implements ItemCostoLogisticoDAOLocal{
	...
	public List<CostoLogisticoDTO> getCostosLogisticos(Long idCliente, List<Long> documentos, String terminoIncoterm, String puerto, String puertos, Long idCurrency, String pais){
		...
		String query1="select 1 as tipo, ccl.nombre as categoria, icl.nombre as item, icl.descripcion, dn.cantidad_contenedores_de_20 as cantidad, dn.cantidad_contenedores_de_20 * case when icl.id_moneda='USD' then icl.valor else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_Fob baseFob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento "+
				...
				"UNION ALL "+ 
				"select 2, ccl.nombre, icl.nombre, icl.descripcion, sum(dn.cantidad_contenedores_de_40), sum(dn.cantidad_contenedores_de_40 * case when icl.id_moneda='USD' then icl.valor else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_Fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento "+
				"from documentos d inner join documento_x_negociacion dn on d.id=dn.id_documento inner join item_costo_logistico icl on 1=1 inner join categorias_costos_logisticos ccl on icl.id_categoria=ccl.id "+
				"where icl.tipo=2 and id_cliente=:idCliente and d.id in ("+documento+") and icl.aplica_"+terminoIncoterm+" and (icl.nombre_puerto_nal IS NULL "+(puerto!=null?" or icl.nombre_puerto_nal= :puertoNal":"")+") and (icl.id_pais_destino IS NULL "+(pais!=null?"or icl.id_pais_destino= :pais":"")+") group by 1,2,3,4,7,8,campo_acumula,consecutivoDocumento  "+
				"UNION ALL "+
				"select 3, ccl.nombre, icl.nombre, icl.descripcion, sum(pd.cantidad_pallets_item), sum(cantidad_pallets_item * case when icl.id_moneda='USD' then icl.valor else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento "+
				...
				"UNION ALL "+
				"select 4, ccl.nombre, icl.nombre, icl.descripcion, count(1), sum(case when icl.id_moneda='USD' then icl.valor else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento "+
				...
				"UNION ALL "+
				"select 5, ccl.nombre, icl.nombre, icl.descripcion, sum(dl.total_cantidad), sum(dl.total_cantidad * case when icl.id_moneda='USD' then icl.valor else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento "+
				...
				"UNION ALL  "+
				"select 7, ccl.nombre, icl.nombre, icl.descripcion, dn.total_peso_bruto, case when rcl.id_moneda='USD' then rcl.valor else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency)* rcl.valor end as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_Fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento "+
				...
				"UNION ALL "+
				"select 8, ccl.nombre, icl.nombre, icl.descripcion, sum(dn.total_peso_bruto), sum(dn.total_peso_bruto * case when icl.id_moneda='USD' then icl.valor else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento "+
				"from documentos d inner join documento_x_negociacion dn on d.id=dn.id_documento inner join item_costo_logistico icl on 1=1 inner join categorias_costos_logisticos ccl on icl.id_categoria=ccl.id "+
				...
				"UNION ALL  "+
				"select 9, ccl.nombre, icl.nombre, icl.descripcion, sum(dn.total_peso_neto), sum(dn.total_peso_neto * case when icl.id_moneda='USD' then icl.valor else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento "+
				"from documentos d inner join documento_x_negociacion dn on d.id=dn.id_documento inner join item_costo_logistico icl on 1=1 inner join categorias_costos_logisticos ccl on icl.id_categoria=ccl.id "+
				...
				"UNION ALL  "+
				"select 10 as tipo, ccl.nombre, icl.nombre, icl.descripcion, dn.cantidad_contenedores_de_20+dn.cantidad_contenedores_de_40 as cantidad, (dn.cantidad_contenedores_de_20 +dn.cantidad_contenedores_de_40) * case when icl.id_moneda='USD' then icl.valor else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select source_destination_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento "+
				"from documentos d inner join documento_x_negociacion dn on d.id=dn.id_documento inner join item_costo_logistico icl on 1=1 inner join categorias_costos_logisticos ccl on icl.id_categoria=ccl.id "+
