...
+
		...
		query1+="select 3 as tipo, ccl.nombre as categoria, icl.nombre as item, icl.descripcion, sum(pd.cantidad_pallets_item) as cantidad, sum(cantidad_pallets_item * case when icl.id_moneda='USD' then icl.valor else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_fob as baseFob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento, ccl.orden, ccl.id as categoriaId, icl.id as itemId "+
				...
				"UNION ALL "+
				"select 4, ccl.nombre, icl.nombre, icl.descripcion, count(1), sum(case when icl.id_moneda='USD' then icl.valor else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento, ccl.orden, ccl.id as categoriaId, icl.id as itemId "+
				...
				"UNION ALL "+
				"select 5, ccl.nombre, icl.nombre, icl.descripcion, sum(dl.total_cantidad), sum(dl.total_cantidad * case when icl.id_moneda='USD' then icl.valor else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento, ccl.orden, ccl.id as categoriaId, icl.id as itemId "+
				...
				"UNION ALL  "+
				"select 7, ccl.nombre, icl.nombre, icl.descripcion, dn.total_peso_bruto, dn.total_peso_bruto * case when rcl.id_moneda='USD' then rcl.valor else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency)* rcl.valor end as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_Fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento, ccl.orden, ccl.id as categoriaId, icl.id as itemId "+
				...
				"UNION ALL "+
				"select 8, ccl.nombre, icl.nombre, icl.descripcion, sum(dn.total_peso_bruto), sum(dn.total_peso_bruto * case when icl.id_moneda='USD' then icl.valor else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento, ccl.orden, ccl.id as categoriaId, icl.id as itemId "+
				"from documentos d inner join documento_x_negociacion dn on d.id=dn.id_documento inner join item_costo_logistico icl on 1=1 inner join categorias_costos_logisticos ccl on icl.id_categoria=ccl.id "+
				...
				"UNION ALL  "+
				"select 9, ccl.nombre, icl.nombre, icl.descripcion, sum(dn.total_peso_neto), sum(dn.total_peso_neto * case when icl.id_moneda='USD' then icl.valor else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency)* icl.valor end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento, ccl.orden, ccl.id as categoriaId, icl.id as itemId "+
				"from documentos d inner join documento_x_negociacion dn on d.id=dn.id_documento inner join item_costo_logistico icl on 1=1 inner join categorias_costos_logisticos ccl on icl.id_categoria=ccl.id "+
		...
		String query2="SELECT * FROM ("+query1+
				"UNION ALL "+ 
				"select 6 as tipo, ccl.nombre, icl.nombre, icl.descripcion, icl.valor, 0, case when icl.id_moneda='USD' then icl.valor_minimo else (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) * icl.valor_minimo end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, '' as consecutivoDocumento, ccl.orden, ccl.id as categoriaId, icl.id as itemId "+ 
