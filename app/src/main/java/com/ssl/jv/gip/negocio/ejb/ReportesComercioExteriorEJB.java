...
public class ReportesComercioExteriorEJB implements ReportesComercioExteriorEJBLocal {
  ...
  @Override
  public List<ProductosXDocumento> consultarProductosPorDocumentoLE(Long id) {
	LOGGER.debug("Metodo: <<consultarProductosPorDocumentoLE>>");
	if (id == null) {
	  throw new IllegalArgumentException("El parametro <<id>> es requerido");
	} else {
	  Map<String, Object> parametros = new HashMap<>();
	  parametros.put("idDocumento", id);
	  return productosXDocumentoDAO.buscarPorConsultaNombrada(ProductosXDocumento.BUSCAR_PRODUCTOS_X_DOCUMENTO_LE, parametros);
	}
  }
}
