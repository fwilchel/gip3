...
public class DocumentoDAO extends GenericDAO<Documento> implements DocumentoDAOLocal {
  ...
  public List<DatoContribucionCafeteraDTO> consultarDatosContribucionCafetera(Map<String, Object> parametros) {
    List<DatoContribucionCafeteraDTO> lista = new ArrayList<>();
    String sql = "";
    ...
    sql = "select " + "distinct(doc_lot.id_documento) as doc_lot, " + "docFX.id, " + "docFX.consecutivo_documento, " + "docFX.fecha_generacion" + " from documentos docFX " + " inner join documentos docLE on docFX.observacion_documento=docLE.consecutivo_documento " + " inner join documentos docFP on docLE.observacion_documento=docFP.consecutivo_documento " + " left outer join documento_x_lotesoic  doc_lot on docFP.id=doc_lot.id_documento " + " where docFX.id_tipo_documento= " + tipo
            + " AND docFX.id_estado = " + estado + " and (doc_lot.contribucion  is null or doc_lot.contribucion=0 or doc_lot.dex is null or doc_lot.dex =0)" + " and  docFX.fecha_generacion>='2015-01-01'" + " ORDER BY docFX.id DESC";
  ...
  public void actualizarDocumentoPorNegociacion(DocumentoIncontermDTO documento) {
    try {
      ...
      sql.append("UPDATE Documento_x_Negociacion SET " + " costo_entrega = " + documento.getCostoEntrega() + " , costo_flete = " + documento.getCostoFlete() + " , costo_seguro = " + documento.getCostoSeguro() + " , otros_gastos = " + documento.getOtrosGastos());
  ...
  public void actualizarEstadoDocumento(DocumentoIncontermDTO documento) {
    try {
      StringBuilder sql = new StringBuilder();
      sql.append("UPDATE documentos SET " + " id_estado = " + documento.getIdEstado() + " WHERE  id = " + documento.getIdDocumento());
  ...
  public void actualizarEstadoDocumentoPorConsecutivo(DocumentoIncontermDTO documento) {
    try {
      StringBuilder sql = new StringBuilder();
      sql.append("UPDATE documentos SET " + " id_estado = " + documento.getIdEstado() + " WHERE  consecutivo_documento = " + documento.getConsecutivoDocumento());
  ...
  public List<DocumentoIncontermDTO> consultarDocumentosSolicitudPedido() {
    ...
    if (listado != null) {
      for (Object[] objs : listado) {
        ...
        dto.setClientesNit(objs[32] != null ? objs[32].toString() : null);
  ...
  public List<DocumentoIncontermDTO> consultarDocumentosSolicitudPedido(FiltroConsultaSolicitudDTO filtro) {
    ...
    if (filtro.getFechaBusqueda() != null) {
      ...
      String strFechaBusqueda = fechaHora.format(filtro.getFechaBusqueda());

      sql.append(" AND documentos.fecha_generacion between '" + strFechaBusqueda + "' and '" + strFechaBusqueda + " 23:59:59' ");
    }

    if (filtro.getConsecutivoDocumento() != null && !filtro.getConsecutivoDocumento().isEmpty()) {
      sql.append(" AND UPPER(documentos.consecutivo_documento) LIKE UPPER('%" + filtro.getConsecutivoDocumento() + "%')");
    }

    if (filtro.getNumeroPedido() != null && !filtro.getNumeroPedido().isEmpty()) {
      sql.append(" AND UPPER(documentos.documento_cliente) LIKE UPPER('%" + filtro.getNumeroPedido() + "%')");
    }

    if (filtro.getNombreCliente() != null && !filtro.getNombreCliente().isEmpty()) {
      sql.append(" AND UPPER(clientes.nombre) LIKE UPPER('%" + filtro.getNombreCliente() + "%')");
    }
    ...
    if (listado != null) {
      for (Object[] objs : listado) {
        ...
        dto.setCantidadDiasVigencia(objs[33] != null ? new Integer(objs[33].toString()) : null);

        dto.setDescripcionMetodoPago(objs[34] != null ? objs[34].toString() : null);

        dto.setDescripcionInglesMetodoPago(objs[35] != null ? objs[35].toString() : null);
  ...
  public List<DocumentoIncontermDTO> consultarDocumentosGeneral(FiltroConsultaSolicitudDTO filtro) {
    ...
    if (filtro.getFechaInicio() != null) {
      ...
      String strFechaInicial = fechaHoraI.format(filtro.getFechaInicio());
      sql.append(" AND documentos.fecha_generacion >= '" + strFechaInicial + " 00:00:00'");
    }

    if (filtro.getFechaFinal() != null) {
      DateFormat fechaHoraF = new SimpleDateFormat("yyyy-MM-dd");
      String strFechaFinal = fechaHoraF.format(filtro.getFechaFinal());
      sql.append(" AND documentos.fecha_generacion <= '" + strFechaFinal + " 23:59:59'");
    }
    ...
    if (filtro.getIdEstado() != null && filtro.getIdEstado().compareTo(new Long(0)) != 0) {
      sql.append(" AND documentos.id_estado = " + filtro.getIdEstado());
    }

    if (filtro.getProveedor() != null && !filtro.getProveedor().trim().isEmpty()) {
      sql.append(" AND documentos.id_proveedor = '" + filtro.getProveedor() + "'");
    }
  ...
  public List<DocumentoIncontermDTO> consultarDocumentosAprobarSolicitudPedido() {
    ...
    List<Object[]> listado = em.createNativeQuery(sql.toString()).setParameter("estado1", (long) ConstantesDocumento.VERIFICADO).getResultList();
  ...
  public List<Documento> consultarDocumentosSolicitudPedido(String consecutivoDocumento) {
    ...
    String sql;
    // Consulta las SPs q estan en estado Verificado (14) de mercadeo y Asignada (16) de Cafe,
    sql = "SELECT d FROM Documento d "
            ...
            + "WHERE UPPER(d.consecutivoDocumento) LIKE UPPER(:consecutivo) "
            + "AND (d.estadosxdocumento.id.idTipoDocumento = :tipoDocumento AND e.id=:verificado and dxn.solicitudCafe = :solicitudCafe) " 
            + "OR (d.estadosxdocumento.id.idTipoDocumento = :tipoDocumento AND e.id=:asignada and dxn.solicitudCafe = :solicitudMercadeo) " 
            + "AND d.consecutivoDocumento NOT IN (SELECT d2.observacionDocumento FROM Documento d2 WHERE d2.observacionDocumento IN (SELECT d3.consecutivoDocumento FROM Documento d3 WHERE d3.estadosxdocumento.id.idTipoDocumento = :tipoDocumento AND d3.estadosxdocumento.estado.id IN (:verificado, :asignada))) "
            + "ORDER BY d.id DESC";
    Query query = em.createQuery(sql);
    query.setParameter("tipoDocumento", (long) ConstantesTipoDocumento.SOLICITUD_PEDIDO);
    query.setParameter("verificado", (long) ConstantesDocumento.VERIFICADO);
    query.setParameter("asignada", (long) ConstantesDocumento.ASIGNADA);
    ...
    try {
      ...
      if (listado != null) {
        for (Object[] objs : listado) {
          ...
          dto.setSolicitudCafe(objs[35].toString() == "t" ? true : false);
          dto.setIdEstado(objs[6] != null ? objs[6].toString() : null);
          dto.setFechaEsperadaEntrega((Timestamp) (objs[24] != null ? objs[24] : null));
          dto.setIdProveedor(objs[42] != null ? objs[42].toString() : null);
          dto.setIdUbicacionOrigen(objs[36] != null ? objs[36].toString() : null);
          dto.setIdUbicacionDestino(objs[37] != null ? objs[37].toString() : null);
          dto.setIdTerminoIncoterm(objs[13] != null ? objs[13].toString() : null);
          dto.setCostoEntrega(objs[15] != null ? new BigDecimal(objs[15].toString()) : null);
          dto.setCostoSeguro(objs[16] != null ? new BigDecimal(objs[16].toString()) : null);
          dto.setCostoFlete(objs[17] != null ? new BigDecimal(objs[17].toString()) : null);
          dto.setOtrosGastos(objs[18] != null ? new BigDecimal(objs[18].toString()) : null);
          dto.setCantidadDiasVigencia(objs[29] != null ? objs[29].toString() : null);
          dto.setTotalPesoNeto(objs[20] != null ? Double.valueOf(objs[20].toString()) : null);
          dto.setTotalPesoBruto(objs[21] != null ? Double.valueOf(objs[21].toString()) : null);
          dto.setTotalTendidos(objs[22] != null ? Double.valueOf(objs[22].toString()) : null);
          dto.setIdTipoDocumento(objs[5] != null ? objs[5].toString() : null);

          clienteDTO.setId(objs[4] != null ? objs[4].toString() : null);
          clienteDTO.setDocumento(objs[3] != null ? objs[3].toString() : null);
  ...
  public List<Documento> consultarOrdenesDeDespacho(String consecutivoDocumento) {
    ...
    try {
      ...
      listado = em.createQuery(query).setParameter("tipoDocumento", (long) ConstantesTipoDocumento.ORDEN_DESPACHO).setMaxResults(35).getResultList();
    } catch (Exception e) {
  ...
  public List<ProductoImprimirLEDTO> consultarProductoListaEmpaque(String strConsecutivoDocumento) {
    ...
    try {
      ...
      String query = "select pi.sku ," + " case when c.modo_factura=1 then pi_ce.descripcion when c.modo_factura=2 then pi.nombre when c.modo_factura=3 then pi_ce.nombre_prd_proveedor end as nombre_producto," + " pxd.cantidad1 , pxd.total_peso_neto_item," + " pxd.total_peso_bruto_item ,pxd.cantidad_cajas_item ,  pxd.cantidad_x_embalaje," + " dxl.consecutivo ," + " pxd.cantidad_pallets_item ," + " pxc_ce.reg_sanitario ,"
              + " (pxd.cantidad1/pi_ce.cantidad_x_embalaje)/pi_ce.total_cajas_x_pallet AS TOTAL_CAJAS_PALLET ," + " case when c.modo_factura=0 then tl.descripcion_ingles when c.modo_factura=1 then tl.descripcion when c.modo_factura=2 then tl.descripcion end as DESCRIPCION_LOTE" + " from productosXdocumentos pxd  " + " LEFT JOIN productos_inventario pi on  pxd.id_producto=pi.id" + " LEFT JOIN productos_inventario_comext pi_ce ON pi_ce.id_producto=pxd.id_producto"
              + " LEFT JOIN documentos d on d.id=pxd.id_documento" + " LEFT join productos_x_cliente_comext pxc_ce on pxc_ce.id_producto=pxd.id_producto and pxc_ce.id_cliente=d.id_cliente" + " LEFT join tipo_loteoic tl on pi_ce.id_tipo_loteoic=tl.id " + " LEFT join documento_x_lotesoic dxl on dxl.id_documento= " + " (select documentos.id from documentos where documentos.consecutivo_documento=(select documentos.observacion_documento from documentos where documentos.consecutivo_documento='"
              + strConsecutivoDocumento + "'))" + " and dxl.id_tipo_lote=pi_ce.id_tipo_loteoic" + " LEFT JOIN unidades on unidades.id = pi.id_uv LEFT JOIN clientes c on c.id=d.id_cliente WHERE d.consecutivo_documento='" + strConsecutivoDocumento + "' order by dxl.consecutivo";
      ...
      if (listado != null) {
        for (Object[] objs : listado) {
          ...
          dto.setCantidad(objs[2] != null ? new BigDecimal(objs[2].toString()) : null);
          dto.setPesoNeto(objs[3] != null ? new BigDecimal(objs[3].toString()) : null);
          dto.setPesoBruto(objs[4] != null ? new BigDecimal(objs[4].toString()) : null);
          dto.setCantidadCajas(objs[5] != null ? new BigDecimal(objs[5].toString()) : null);
          dto.setCantidadPorEmbalaje(objs[6] != null ? new BigDecimal(objs[6].toString()) : null);
          dto.setConsecutivoLote(objs[7] != null ? objs[7].toString() : null);
          dto.setPallet(objs[8] != null ? new BigDecimal(objs[8].toString()) : null);
          dto.setRegistroSanitario(objs[9] != null ? objs[9].toString() : null);
          dto.setCajasPorPallets(objs[10] != null ? new BigDecimal(objs[10].toString()) : null);
          ...
          // .toString() : null);
          dto.setDescripcionLote(objs[11] != null ? objs[11].toString() : null);
  ...
  public FacturaDirectaDTO consultarDocumentoFacturaDirecta(String strConsecutivoDocumento) {
     ...
     * .toString()) : null);
     * 
     * dto.setValorIva10(objs[11] != null ? new BigDecimal(objs[11] .toString())
     * : null);
     * 
     * dto.setValorIva16(objs[12] != null ? new BigDecimal(objs[12] .toString())
     * : null);
     * 
     * dto.setValorIva5(objs[13] != null ? new BigDecimal(objs[13] .toString())
     * : null);
     * 
     * 
     * dto.setValorTotal(objs[14] != null ? new BigDecimal(objs[14] .toString())
     * : null);
     * 
     * } }
     */
  ...
  public BigInteger generarListaEmpaque(ListaEmpaqueDTO listaEmpaqueDTO) {
    ...
    BigInteger secuence = (BigInteger) em.createNativeQuery(sqlSec).getSingleResult();
    ...
    String sqlEmpresa = "select id_empresa from ubicaciones where id = " + listaEmpaqueDTO.getIdUbicacionOrigen() + " or id = " + listaEmpaqueDTO.getIdUbicacionDestino();
    BigInteger prefijoEmpresa = (BigInteger) em.createNativeQuery(sqlEmpresa).getSingleResult();
  ...
  @SuppressWarnings("unchecked")
  @Override
  public List<Documento> consultarDocumentosPorTipoDocumentoYEstado(FiltroDocumentoDTO filtro) {
    Query query = em.createNamedQuery(Documento.FIND_BY_TIPO_DOCUMENTO_AND_ESTADO);
  ...
  @SuppressWarnings("unchecked")
  @Override
  public List<Documento> consultarDocumentosPorTipoDocumentoConsecutivoDocumentoYEstados(Long idTipoDocumento, String consecutivoDocumento, Long... idEstados) {
    Query query = em.createNamedQuery(Documento.LISTA_EMPAQUE_FIND_BY_ESTADO_AND_TIPO_DOCUMENTO_AND_CONSECUTIVO);
  ...
  public List<Documento> consultarDocumentosFacturaExportacion(String consecutivoDocumento) {
    ...
    try {
      ...
      listado = em.createQuery(query).setParameter("tipoDocumento", (long) ConstantesTipoDocumento.FACTURA_EXPORTACION).getResultList();
    } catch (Exception e) {
      LOGGER.error(e + "********Error consultando Documentos por tipo de documento ORDEN DE DESPACHO");
      return null;
    }
  ...
  @Override
  public List<Documento> consultarDocumentosPorTipoDocumentoYEstados(FiltroDocumentoDTO filtro) {
    Query query = em.createNamedQuery(Documento.FIND_BY_TIPO_DOCUMENTO_AND_ESTADOS);
    query.setParameter("idTipoDocumento", filtro.getIdTipoDocumento());
    query.setParameter("idEstado", filtro.getIdEstado());
  ...
  public List<AutorizarDocumentoDTO> consultarDocumentosAutorizar(String consecutivoDocumento) {
    ...
    sql = sql + " AND UPPER(documentos.consecutivo_documento) LIKE UPPER('" + consecutivoDocumento + "')" + " ORDER BY documentos.id DESC";
  ...
  public void anularSolicitudPedido(Documento documento) {
    LOGGER.trace("Metodo: <<anularSolicitudPedido>> parametros / documento ->> {" + documento + "} ");
  ...
  public List<DocumentoInstruccionEmbarqueDTO> consultarDocumentosInstruccionEmbarque(Long idCliente) {
    ...
    if (listado != null) {
      for (Object[] objs : listado) {
        ...
        dto.setDocumentoCliente(objs[3] != null ? objs[3].toString() : null);
        dto.setIdCliente(objs[4] != null ? new Long(objs[4].toString()) : null);
        dto.setIdTipoDocumento(objs[5] != null ? new Long(objs[5].toString()) : null);
        dto.setIdEstado(objs[6] != null ? new Long(objs[6].toString()) : null);
        dto.setNombreCliente(objs[7] != null ? objs[7].toString() : null);
        dto.setIdCliente(objs[8] != null ? new Long(objs[8].toString()) : null);
        ...
        dto.setIdIncoterm(objs[13] != null ? new Long(objs[13].toString()) : null);
        dto.setNombreIncoterm(objs[14] != null ? objs[14].toString() : null);
        dto.setCostoEntrega(objs[15] != null ? new Double(objs[15].toString()) : null);
        dto.setCostoSeguro(objs[16] != null ? new Double(objs[16].toString()) : null);
        dto.setCostoFlete(objs[17] != null ? new Double(objs[17].toString()) : null);
        dto.setOtrosGastos(objs[18] != null ? new Double(objs[18].toString()) : null);
        dto.setObservacionesMarcacion2(objs[19] != null ? objs[19].toString() : null);
        dto.setTotalPesoNeto(objs[20] != null ? new Double(objs[20].toString()) : null);
        dto.setTotalPesoBruto(objs[21] != null ? new Double(objs[21].toString()) : null);
        dto.setTotalTendidos(objs[22] != null ? new Double(objs[22].toString()) : null);
        dto.setTotalPallets(objs[23] != null ? new Double(objs[23].toString()) : null);
        dto.setFechaEsperadaEntrega(objs[24] != null ? (Date) objs[24] : null);
        dto.setCantidadContenedoresDe20(objs[25] != null ? new Double(objs[25].toString()) : null);
        dto.setCantidadContenedoresDe40(objs[26] != null ? new Double(objs[26].toString()) : null);
        dto.setLugarIncoterm(objs[27] != null ? objs[27].toString() : null);
        dto.setNombreCiudad(objs[28] != null ? objs[28].toString() : null);
        dto.setCantidadDiasVigencia(objs[29] != null ? new Long(objs[29].toString()) : null);
        dto.setDescripcionPago(objs[30] != null ? objs[30].toString() : null);
        dto.setObsrevacionDocumento(objs[31] != null ? objs[31].toString() : null);
        dto.setDescripcionIngles(objs[32] != null ? objs[32].toString() : null);
        dto.setSolicitudCafe(objs[33] != null ? (Boolean) objs[33] : false);
  ...
  @Override
  public List<Documento> consultarDocumentosPorTipoDocumentoYFechas(FiltroDocumentoDTO filtro) {
    Query query = em.createNamedQuery(Documento.FIND_BY_FECHAS_TIPO_DOCUMENTO);
    query.setParameter("idTipoDocumento", filtro.getIdTipoDocumento());
  ...
  public List<Documento> consultarDocumentosParaGenerarFacturaExportacion(String consecutivoDocumento) {
    ...
    query.setParameter("idTipoDocumento", Long.valueOf(ConstantesTipoDocumento.LISTA_EMPAQUE));
    query.setParameter("idEstado1", com.ssl.jv.gip.util.Estado.ACTIVO.getCodigo());
  ...
  public List<Documento> consultarDocumentosDespacharMercancia(String consecutivoDocumento) {
    ...
    try {
      ...
      listado = em.createQuery(query).setParameter("estado", (long) ConstantesDocumento.ACTIVO).setParameter("tipoDocumento", (long) ConstantesTipoDocumento.VENTA_DIRECTA).setParameter("consecutivo", consecutivoDocumento.equals("") ? "%" : "%" + consecutivoDocumento + "%").setMaxResults(35).getResultList();
    } catch (Exception e) {
      LOGGER.error(e + "********Error consultando Documentos por tipo de documento VENTA DIRECTA");
      return null;
    }
    return listado;
  }
  ...
  public List<Documento> consultarDocumentosPorTipoDocumentoEstadoTipoCafe(FiltroDocumentoDTO filtro) {
    Query query = em.createNamedQuery(Documento.FIND_BY_TIPO_DOCUMENTO_AND_ESTADO_AND_TIPO_CAFE);
    query.setParameter("idTipoDocumento", filtro.getIdTipoDocumento());
    query.setParameter("idEstado", filtro.getIdEstado());
    return query.getResultList();
  }
  ...
  @Override
  public List<Documento> consultarDocumentosPorTipoDocumentoEstadoSolicitudCafeAndConsecutivo(FiltroDocumentoDTO filtro) {
    Query query = em.createNamedQuery(Documento.FIND_BY_TIPO_DOCUMENTO_AND_ESTADO_AND_SOLICITUD_CAFE_AND_CONSECUTIVO);
    query.setParameter("idTipoDocumento", filtro.getIdTipoDocumento());
    query.setParameter("idEstado", filtro.getIdEstado());
  ...
  public String modificarFacturaProforma(DocumentoIncontermDTO documento, List<ProductoPorClienteComExtDTO> listado) {
    ...
    // ConstantesDocumento.ANULADO + " WHERE  observacion_documento = " +
    ...
    //
    // String sqlAuditoria =
    // "SELECT  log_auditoria.id_funcionalidad,log_auditoria.id_reg_tabla,log_auditoria.valor_anterior "
    ...
    // em.createNativeQuery(sqlAuditoria).getResultList();
    // if (auditoria != null && auditoria.size() > 0) {
    // String estado = auditoria.get(0)[2] != null ?
    // auditoria.get(0)[2].toString().trim() : null;
    //
    // String sqlDocumento = "UPDATE documentos SET valor_total = " +
    // documento.getValorTotalDocumento().doubleValue() +
    // ", fecha_esperada_entrega = '" + documento.getFechaEsperadaEntrega() +
    // "' ,id_estado = " + estado + " WHERE  id = " + documento.getIdDocumento()
    // + " ";
    //
    // em.createNativeQuery(sqlDocumento).executeUpdate();
    //
    // String sql1 = "UPDATE Documento_x_Negociacion SET id_termino_incoterm = "
    // + documento.getIdTerminoIncoterm() + ", "
    // + "total_peso_neto = " + documento.getTotalPesoNeto().doubleValue() +
    // ", total_peso_bruto = " + documento.getTotalPesoBruto().doubleValue() +
    // ", "
    // + "total_tendidos = " + documento.getTotalTendidos().doubleValue() + ", "
    // + "total_pallets = " + documento.getTotalPallets().doubleValue() +
    // ", cantidad_contenedores_de_20 = " +
    // documento.getCantidadContenedores20().doubleValue() + ", "
    // + "cantidad_contenedores_de_40 = " +
    // documento.getCantidadContenedores40().doubleValue() +
    // ", lugar_incoterm = '" + documento.getLugarIncoterm() + "' "
    // + "WHERE  id_documento = " + documento.getIdDocumento() + "";
    //
    // em.createNativeQuery(sql1).executeUpdate();
    //
    // String sql2 = "UPDATE Documento_x_Negociacion SET costo_entrega = " +
    // documento.getCostoEntrega().doubleValue() + ", costo_flete = " +
    // documento.getCostoFlete().doubleValue() + ", "
    // + "costo_seguro = " + documento.getCostoSeguro().doubleValue() +
    // ", otros_gastos = " + documento.getOtrosGastos().doubleValue() + " "
    // + "WHERE  id_documento = " + documento.getIdDocumento() + "";
    //
    ...
    for (ProductoPorClienteComExtDTO dto : listado) {
      ...
      if (dto.isBlnIncluirBusqueda()) {
        ...
        String sql = "select id_documento, id_producto from productosXdocumentos " + "where id_documento = " + documento.getIdDocumento() + "  AND id_producto = " + dto.getIntIdProductoInventario();

        List<Object[]> regExiste = em.createNativeQuery(sql).getResultList();

        if (regExiste != null && regExiste.size() > 0) {
          String sqlModificarFacturaProforma = "UPDATE productosXdocumentos SET " + "total_peso_neto_item = " + dto.getDblTotalPesoNeto() + ", " + "total_peso_bruto_item = " + dto.getDblTotalPesoBruto() + ", " + "valor_unitario_usd = " + dto.getDblPrecioUSD() + ", " + "cantidad1 = " + dto.getDblCantidad1ProductoxDocumento() + " , " + "valor_total = " + dto.getDblValorTotalProductoxDocumento() + ", " + "cantidad_cajas_item = " + dto.getDblTotalCajas() + ", " + "cantidad_pallets_item = "
          ...
          String sqlInsert = "INSERT INTO productosXdocumentos (" + "id_documento, " + "id_producto, " + "cantidad1, " + "fecha_estimada_entrega, " + "fecha_entrega, " + "id_ml, " + "id_unidades, " + "total_peso_neto_item, " + "valor_unitario_usd, " + "valor_total, " + "total_peso_bruto_item, " + "cantidad_cajas_item, " + "cantidad_pallets_item, " + "cantidad_x_embalaje" + ") " + "VALUES (" + documento.getIdDocumento() + ", " + dto.getIntIdProductoInventario() + ", "
                  + dto.getDblCantidad1ProductoxDocumento() + ", " + "current_timestamp + '2 days', " + "current_timestamp + '2 days', " + "'USD', " + "(select id_ud from productos_inventario where id = " + dto.getIntIdProductoInventario() + "), " + dto.getDblTotalPesoNeto() + ", " + dto.getDblPrecioUSD() + ", " + dto.getDblValorTotalProductoxDocumento() + ", " + dto.getDblTotalPesoBruto() + ", " + dto.getDblTotalCajas() + ", " + dto.getDblTotalCajasPallet() + ", "
        ...
        String sqlEliminar = "DELETE FROM productosXdocumentos WHERE id_producto = " + dto.getIntIdProductoInventario() + " " + "AND id_documento = " + documento.getIdDocumento() + "";
  ...
  public void actualizarEstadoDocumentoPorId(Documento documento) {
    try {
      StringBuilder sql = new StringBuilder();
      sql.append("UPDATE documentos SET id_estado = " + documento.getEstadosxdocumento().getEstado().getId() + " WHERE id = " + documento.getId());
  ...
  public List<Documento> consultarDocumentosPorTipoDocumentoEstadoSolicitudCafeFechas(FiltroDocumentoDTO filtro) {
    Query query = em.createNamedQuery(Documento.FIND_BY_FECHAS_AND_TIPO_DOCUMENTO_AND_ESTADO_AND_SOLICITUD_CAFE);
    query.setParameter("idTipoDocumento", filtro.getIdTipoDocumento());
    query.setParameter("idEstado", filtro.getIdEstado());
    query.setParameter("solicitudCafe", filtro.isSolicitudCafe());
    query.setParameter("fechaInicio", filtro.getFechaInicio());
  ...
  @Override
  public List<Documento> consultarDocumentosPorObservacionDocumento(String observacionDocumento) {
    Query query = em.createNamedQuery(Documento.FIND_BY_OBSERVACION_DOCUMENTO);
    query.setParameter("observacionDocumento", observacionDocumento);
    return query.getResultList();
  }
  ...
  public List<Documento> consultarDocumentosFacturaExportacionEstado() {
    ...
    try {
      ...
      listado = em.createQuery(query).setParameter("estadoDocumento", (long) ConstantesDocumento.ANULADO).setParameter("tipoDocumento", (long) ConstantesTipoDocumento.FACTURA_EXPORTACION).setMaxResults(20).getResultList();

    } catch (Exception e) {
      LOGGER.error(e + "********Error consultando Documentos por tipo de documento FACTURA_EXPORTACION y estado");
      return null;
    }
  ...
  public List<Documento> consultarDocumentosFacturaExportacionEstado(String consecutivoDocumento) {
    ...
    try {
      ...
      listado = em.createQuery(query).setParameter("estadoDocumento", (long) ConstantesDocumento.ANULADO).setParameter("tipoDocumento", (long) ConstantesTipoDocumento.FACTURA_EXPORTACION).setParameter("consecutivo", consecutivoDocumento.equals("") ? "%" : "%" + consecutivoDocumento + "%").setMaxResults(20).getResultList();

    } catch (Exception e) {
      LOGGER.error(e + "********Error consultando Documentos por tipo de documento FACTURA_EXPORTACION, estado y consecutivo");
      return null;
    }
    return listado;
  }
  ...
  public List<DocumentoRecibirDevolucionDTO> consultarDocumentosRecibirDevolucion(String bodega) {
    ...
    try {
      ...
      List<Object[]> listadoInicial = em.createNativeQuery(query).getResultList();
      ...
      LOGGER.error(e + "********Error DocumentoRecibirDevolucionDTO");
      return null;
    }
    return listado;
  }
  ...
  public List<Documento> consultarDocumentos(FiltroDocumentoDTO filtro) {
    ...
    buildFromStatement(from, filtro);
    StringBuilder where = new StringBuilder();
    where.append("WHERE 1=1 ");
    buildWhereStatement(where, filtro);
    StringBuilder orderBy = new StringBuilder();
   ...
   * @param filtro
   */
  public void buildFromStatement(StringBuilder from, FiltroDocumentoDTO filtro) {
    LOGGER.trace("Metodo: <<buildFromStatement (filtro)>>");
    if (filtro != null) {
      // if (filtro.getCliente() != null) {
      from.append("JOIN d.cliente cliente ");
      // } 
      // d.ubicacionOrigen ubicacionOrigen
      // if (filtro.getPuntoVenta() != null) {
      from.append("LEFT JOIN d.puntoVenta puntoVenta ");
      // }
      // if (filtro.getUbicacionOrigen() != null) {
      from.append("LEFT JOIN d.ubicacionOrigen ubicacionOrigen ");
      // }
      // if (filtro.getUbicacionDestino() != null) {
      from.append("LEFT JOIN d.ubicacionDestino ubicacionDestino ");
      // }
      // if (filtro.isSolicitudCafe() != null) {
      from.append("LEFT JOIN d.documentoXNegociacions dxn ");
      // }
    }
  }
   ...
   * @param filtro
   */
  public void buildWhereStatement(StringBuilder where, FiltroDocumentoDTO filtro) {
    LOGGER.trace("Metodo: <<buildWhereStatement (filtro)>>");
    if (filtro != null) {
      if (filtro.getConsecutivoDocumento() != null && !filtro.getConsecutivoDocumento().isEmpty()) {
        where.append("AND UPPER(d.consecutivoDocumento) LIKE UPPER(:consecutivoDocumento) ");
      }
      ...
      if (filtro.getFechaGeneracion() != null) {
        where.append("AND d.fechaGeneracion = :fechaGeneracion ");
      }
      ...
      if (filtro.getCliente() != null && filtro.getCliente() != null) {
        where.append("AND d.cliente.id IN (:cliente) ");
      }
      if (filtro.getPuntoVenta() != null && filtro.getPuntoVenta() != null) {
  ...
  public void setParameters(FiltroDocumentoDTO filtro, Query query) {
    ...
    if (filtro.getFechaGeneracion() != null) {
      query.setParameter("fechaGeneracion", filtro.getFechaGeneracion());
    }
    if (filtro.getFechaGeneracionInicio() != null && filtro.getFechaGeneracionFin() != null) {
