...
public class GenerarFacturaExportacionMB extends UtilMB {
  ...
  public StreamedContent getReportePDF() {
    ...
    parametros.put("ciudad", seleccionado.getCliente().getCiudad() == null ? "" : seleccionado.getCliente().getCiudad().getNombre());
    ...
    if (seleccionado.getCliente().getModoFactura() == 1) {
      parametros.put("metodoPago", seleccionado.getCliente().getMetodoPago() == null ? "" : seleccionado.getCliente().getMetodoPago().getDescripcionIngles());
    } else {
      parametros.put("metodoPago", seleccionado.getCliente().getMetodoPago() == null ? "" : seleccionado.getCliente().getMetodoPago().getDescripcion());
    }
    if (seleccionado.getCliente().getModoFactura() == 1) {
