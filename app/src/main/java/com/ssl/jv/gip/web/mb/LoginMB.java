    ...
    if (usuario != null) {
      ...
      if (usuario.getContrasena().equals(pwd)) {
        ...
        if (h.size() > 0) {
          ...
          if (menu.getTema().equals("cn")) {
            try {
              menu.setPuntoVenta(comunEJB.consultarPuntoVentaPorUsuario(usuario.getId()));
            } catch (Exception ex) {
              LOGGER.error("el usuario no se ha asociado a un punto de venta");
            }
          }
//          String tema = getRequestParameter("tema");
//          if (tema == null) {
//            menu.setTema("default");
//          } else {
//            switch (tema) {
//              case "cn":
//                menu.setTema("cn");
