...
public class ProductoInventarioDAO extends GenericDAO<ProductosInventario> implements ProductoInventarioDAOLocal {
  ...
  public Object[] consultar(ProductosInventario pi, int first, int pageSize, String sortField, SortOrder sortOrder, boolean count) {
	...
	String query = "SELECT " + (count ? "COUNT(pi)" : "pi") + " FROM ProductosInventario pi " + (count ? "" : "LEFT JOIN FETCH pi.productosInventarioComext pic LEFT JOIN FETCH pi.unidadVenta uv LEFT JOIN FETCH pi.unidadDespacho ud LEFT JOIN FETCH pic.unidadEmbalaje ue LEFT JOIN FETCH pic.tipoLoteoic tl LEFT JOIN FETCH pi.categoriasInventario ci1 LEFT JOIN FETCH ci1.categoriasInventario ci2 LEFT JOIN FETCH pi.pais pa ") + " WHERE 1=1 "
		+ (pi.getSku() != null && !pi.getSku().equals("") ? " AND UPPER (pi.sku) LIKE UPPER (:sku) " : "") + (pi.getNombre() != null && !pi.getNombre().equals("") ? " AND UPPER (pi.nombre) LIKE UPPER (:nombre) " : "") + (pi.getCategoriasInventario() != null && pi.getCategoriasInventario().getId() != null && pi.getCategoriasInventario().getId() != 0 ? " AND pi.categoriasInventario= :categoria " : "")
		+ (pi.getPais() != null && pi.getPais().getId() != null && !pi.getPais().getId().equals("") ? " AND pi.pais.id= :idPais " : "") + (pi.getDesactivado() != null ? " AND pi.desactivado= :desactivado " : "") + (!count && sortField != null && !sortField.equals("") ? " ORDER BY pi." + sortField + (sortOrder.equals(SortOrder.UNSORTED) ? "" : sortOrder.equals(SortOrder.ASCENDING) ? " ASC" : " DESC") : "");
  ...
  public List<ProductosInventario> consultarTodos() {
	return em.createNamedQuery(ProductosInventario.PRODUCTOS_INVENTARIO_FIND_ALL).getResultList();
  }

  @SuppressWarnings("unchecked")
  @Override
  public List<ProductosInventario> consultarPorUsuarioCategoriaSKUNombreAndEstado(ProductosInventarioFiltroDTO filtro) {
	Query query = em.createNamedQuery(ProductosInventario.PRODUCTOS_INVENTARIO_FIND_BY_USUARIO_CATEGORIA_SKU_NOMBRE_ESTADO);
	query.setParameter("idUsuario", filtro.getIdUsuario());
	query.setParameter("paramDesactivado", filtro.getEstado() != null ? true : false);
	query.setParameter("desactivado", filtro.getEstado());
	query.setParameter("paramCategoria", filtro.getIdCategoria() != null ? true : false);
	query.setParameter("idCategoria", filtro.getIdCategoria());
	query.setParameter("paramSku", filtro.getSku() != null ? true : false);
	query.setParameter("sku", filtro.getSku() != null ? "%" + filtro.getSku() + "%" : null);
	query.setParameter("paramNombre", filtro.getNombre() != null ? true : false);
	query.setParameter("nombre", filtro.getNombre() != null ? "%" + filtro.getNombre() + "%" : null);
	return query.getResultList();
  }

  @Override
  public List<ProductosInventario> consultarActivos() {
	return em.createNamedQuery(ProductosInventario.PRODUCTOS_INVENTARIO_FIND_ACTIVOS).getResultList();
  }

  @Override
  public ProductosInventario consultarPorSku(String sku) {
	Query query = em.createNamedQuery(ProductosInventario.PRODUCTOS_INVENTARIO_FIND_BY_SKU);
  ...
  public List<ProductosInventario> consultarPorSkus(List<String> skus) {
	Query query = em.createNamedQuery(ProductosInventario.PRODUCTOS_INVENTARIO_FIND_BY_SKUs);
	query.setParameter("skus", skus);
	return query.getResultList();
  }

  @Override
  public List<ProductosInventario> consultarPorEstadoCategoriaSKUNombreAndControlStock(ProductosInventarioFiltroDTO filtro) {
	Query query = em.createNamedQuery(ProductosInventario.PRODUCTOS_INVENTARIO_FIND_BY_DESACTIVADO_CATEGORIA_SKU_NOMBRE_AND_CONTROLSTOCK);
	query.setParameter("desactivado", filtro.getEstado());
	query.setParameter("paramCategoria", filtro.getIdCategoria() != null ? true : false);
	query.setParameter("idCategoria", filtro.getIdCategoria());
	if (filtro.getSku() == null || filtro.getSku().isEmpty()) {
  ...
  public List<ProductoDevolucionDTO> consultarActivosPorPaises(List<String> paises) {
	...
	Query query = em.createNamedQuery(ProductosInventario.PRODUCTOS_INVENTARIO_FIND_ACTIVOS_PAISES);
  ...
  public List<ProductoDevolucionDTO> consultarActivosDev() {
	List<ProductoDevolucionDTO> lista = new ArrayList<ProductoDevolucionDTO>();
	Query query = em.createNamedQuery(ProductosInventario.PRODUCTOS_INVENTARIO_FIND_ACTIVOS_DEV);
