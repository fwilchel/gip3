...
//			LOGGER.debug("Log de auditoria creado con id: " + auditoria.getIdLog());
	...
	public void guardarMovimientosInventarioComercioExterior(List<MovimientosInventarioComext> movimientosInventarioComexts, LogAuditoria auditoria) throws Exception {
		Map<Long, BigDecimal> ultimosSaldosInventario = this.movimientosInventarioComextDAO.getMapUltimosSaldos();
	...
	public void crearProductosXClientesDesdeArchivo(byte[] archivo) throws IOException {
		...
		if (!listaPXC.isEmpty()) {
			for (ProductosXCliente pxc : listaPXC) {
				...
				pk.setIdCliente(pxc.getCliente().getId());
				...
				try {
					...
					Object pxcId;
					{
						...
						pxcId = productoClienteDAO.buscarRegistroPorConsultaNativa(
								"SELECT id FROM productosxcliente pxc WHERE pxc.id_producto = :idProducto AND pxc.id_cliente = :idCliente AND pxc.id_punto_venta = :idPuntoVenta", parametros);
					...
					}
					{
						...
						productoClienteDAO.ejecutarConsultaNativa("" + "UPDATE productosxcliente SET " + "fecha_inicial_vigencia = :fecha_inicial_vigencia, " + "fecha_final_vigencia = :fecha_final_vigencia, "
								+ "precio_ml = :precio_ml, " + "precio_usd = :precio_usd, " + "id_ml = :id_ml, " + "iva = :iva, " + "descuentoxproducto = :descuentoxproducto, "
								+ "otros_descuentos = :otros_descuentos, " + "activo = :activo " + "WHERE id = :id", parametros);
					}
				} catch (Exception ex) {
	...
	public void modificarProductosXClientes(ProductosXCliente pxcOld, ProductosXCliente pxcNew, LogAuditoria auditoria) {
		...
		String[][] matrizOld = {
				{ "Id_Producto", "Id_Cliente", "Fecha_Incial_Vigencia", "Fecha_Final_Vigencia", "Vigente", "Precio_ML", "Precio_USD", "Id_ML", "IVA", "DescuentoxProducto", "Otros_Descuentos",
						"Id_Punto_Venta", "Activo" },
				{ pxcOld.getProductosInventario().getId().toString(), pxcOld.getCliente().getId().toString(), pxcOld.getFechaInicialVigencia().toString(), pxcOld.getFechaFinalVigencia().toString(),
						pxcOld.getVigente().toString(), pxcOld.getPrecioMl().toString(), pxcOld.getPrecioUsd().toString(), pxcOld.getMoneda().getId(), pxcOld.getIva().toString(),
						pxcOld.getDescuentoxproducto().toString(), pxcOld.getOtrosDescuentos().toString(), pxcOld.getPuntoVenta().getId().toString(), pxcOld.getActivo().toString() } };
		String[][] matrizNew = {
				{ "Id_Producto", "Id_Cliente", "Fecha_Incial_Vigencia", "Fecha_Final_Vigencia", "Vigente", "Precio_ML", "Precio_USD", "Id_ML", "IVA", "DescuentoxProducto", "Otros_Descuentos",
						"Id_Punto_Venta", "Activo" },
				{ pxcNew.getProductosInventario().getId().toString(), pxcNew.getCliente().getId().toString(), pxcNew.getFechaInicialVigencia().toString(), pxcNew.getFechaFinalVigencia().toString(),
						pxcNew.getVigente().toString(), pxcNew.getPrecioMl().toString(), pxcNew.getPrecioUsd().toString(), pxcNew.getMoneda().getId(), pxcNew.getIva().toString(),
						pxcNew.getDescuentoxproducto().toString(), pxcNew.getOtrosDescuentos().toString(), pxcNew.getPuntoVenta().getId().toString(), pxcNew.getActivo().toString() } };
