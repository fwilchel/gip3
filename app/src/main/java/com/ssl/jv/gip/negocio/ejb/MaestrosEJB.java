...
public class MaestrosEJB<puntoVentaDAO> implements MaestrosEJBLocal {
  ...
  public Cliente crearCliente(Cliente pEntidad, LogAuditoria auditoria) {
    try {
      ...
      // LOGGER.debug("Log de auditoria creado con id: " +
      // auditoria.getIdLog());
      return pEntidad;
    } catch (Exception e) {
  ...
  public void modificarProductosXClientes(ProductosXCliente pxcOld, ProductosXCliente pxcNew, LogAuditoria auditoria) {
    ...
    String[][] matrizOld = {
      {"Id_Producto", "Id_Cliente", "Fecha_Incial_Vigencia", "Fecha_Final_Vigencia", "Vigente", "Precio_ML", "Precio_USD", "Id_ML", "IVA", "DescuentoxProducto", "Otros_Descuentos", "Id_Punto_Venta", "Activo"},
      {pxcOld.getProductosInventario().getId().toString(), pxcOld.getCliente().getId().toString(), pxcOld.getFechaInicialVigencia().toString(), pxcOld.getFechaFinalVigencia().toString(), pxcOld.getVigente().toString(), pxcOld.getPrecioMl().toString(), pxcOld.getPrecioUsd().toString(), pxcOld.getMoneda().getId(), pxcOld.getIva().toString(), pxcOld.getDescuentoxproducto().toString(), pxcOld.getOtrosDescuentos().toString(), pxcOld.getPuntoVenta().getId().toString(),
        pxcOld.getActivo().toString()}};
    String[][] matrizNew = {
      {"Id_Producto", "Id_Cliente", "Fecha_Incial_Vigencia", "Fecha_Final_Vigencia", "Vigente", "Precio_ML", "Precio_USD", "Id_ML", "IVA", "DescuentoxProducto", "Otros_Descuentos", "Id_Punto_Venta", "Activo"},
      {pxcNew.getProductosInventario().getId().toString(), pxcNew.getCliente().getId().toString(), pxcNew.getFechaInicialVigencia().toString(), pxcNew.getFechaFinalVigencia().toString(), pxcNew.getVigente().toString(), pxcNew.getPrecioMl().toString(), pxcNew.getPrecioUsd().toString(), pxcNew.getMoneda().getId(), pxcNew.getIva().toString(), pxcNew.getDescuentoxproducto().toString(), pxcNew.getOtrosDescuentos().toString(), pxcNew.getPuntoVenta().getId().toString(),
