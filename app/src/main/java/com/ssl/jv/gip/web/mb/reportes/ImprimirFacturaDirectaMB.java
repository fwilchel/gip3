...
public class ImprimirFacturaDirectaMB extends UtilMB {
	...
	public void setSeleccionado(Documento seleccionado) {
		...
		// lotes =
		// comercioEjb.consultarProductoPorDocumentoLoteAsignarLotesOIC(seleccionado.getId(),
		// seleccionado.getCliente().getId());
	}
	...
	public StreamedContent getReportePDF() {
		...
		// parametros.put("descuentoCliente","%");
		// parametros.put("observaciones",
		// "Los productos marcados con (*) incluyen descuento adicional.");
		...
		try {
			...
			reportePDF = new DefaultStreamedContent(new ByteArrayInputStream(os.toByteArray()), "application/pdf ", seleccionado2.getConsecutivoDocumento() + "_copia.pdf");

			/*
			 * String xmlfile =
			 * FacesContext.getCurrentInstance().getExternalContext().
			 * getRealPath("/reportes/report2.jrxml"); JasperReport reporte =
			 * JasperCompileManager.compileReport(xmlfile); JasperPrint print =
			 * JasperFillManager.fillReport(reporte, parametros, ds);
			 * ByteArrayOutputStream baos =new ByteArrayOutputStream();
			 * net.sf.jasperreports
			 * .engine.JasperExportManager.exportReportToPdfStream(print,baos);
			 */
		} catch (Exception e) {
