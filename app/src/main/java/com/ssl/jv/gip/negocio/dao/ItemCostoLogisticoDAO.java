...
public class ItemCostoLogisticoDAO extends GenericDAO<ItemCostoLogistico> implements ItemCostoLogisticoDAOLocal {
  ...
  public List<CostoLogisticoDTO> getCostosLogisticos(Long idCliente, List<Long> documentos, String terminoIncoterm, String puerto, String puertos, Long idCurrency, String pais, Integer tipoContenedor1, BigDecimal cantidad1, Integer tipoContenedor2, BigDecimal cantidad2, BigDecimal valorTotal) {
    ...
    query1 += "select 3 as tipo, ccl.nombre as categoria, icl.nombre as item, icl.descripcion, sum(pd.cantidad_pallets_item) as cantidad, sum(cantidad_pallets_item * case when icl.id_moneda='USD' then icl.valor else icl.valor / (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else icl.valor_minimo / (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) end as valorMinimo, icl.base_fob as baseFob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento, ccl.orden, ccl.id as categoriaId, icl.id as itemId "
        ...
        + "UNION ALL "
        + "select 5, ccl.nombre, icl.nombre, icl.descripcion, count(1), count(1)*(case when icl.id_moneda='USD' then icl.valor else icl.valor / (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) end) as valor, case when icl.id_moneda='USD' then icl.valor_minimo else icl.valor_minimo / (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, d.consecutivo_documento as consecutivoDocumento, ccl.orden, ccl.id as categoriaId, icl.id as itemId "
        ...
        + "UNION ALL  "
        + "select 11, ccl.nombre, icl.nombre, icl.descripcion, 1, case when icl.id_moneda='USD' then icl.valor else icl.valor / (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) end as valor, case when icl.id_moneda='USD' then icl.valor_minimo else icl.valor_minimo / (select destination_source_exchange_rate from facts_currency_conversion where id=:idCurrency) end as valorMinimo, icl.base_fob, ccl.campo_acumula campoAcumula, '' as consecutivoDocumento, ccl.orden, ccl.id as categoriaId, icl.id as itemId "
        + "from item_costo_logistico icl inner join categorias_costos_logisticos ccl on icl.id_categoria=ccl.id inner join documentos d on 1=1 "
        + "where icl.tipo=11 and id_cliente=:idCliente and d.id in (" + documento + ") and icl.aplica_" + terminoIncoterm + " and (icl.nombre_puerto_nal IS NULL " + (puerto != null ? " or icl.nombre_puerto_nal= :puertoNal" : "") + ") and (icl.id_pais_destino IS NULL " + (pais != null ? "or icl.id_pais_destino= :pais" : "") + ") and (icl.nombre_puertos_nal_internal IS NULL " + (puertos != null ? " or icl.nombre_puertos_nal_internal= :puertoInternal" : "") + ")  ";
