...
public class ComercioExteriorEJB implements ComercioExteriorEJBLocal {
          ...
          if (pxdSeleccionado.getProductosInventario().getProductosInventarioComext().getControlStock()) {
            ...
            if (cantidadAnterior.compareTo(cantidadNueva) == 0) {
              ...
              if (cantidadAnterior.compareTo(cantidadNueva) == -1) {
                // significa q aumento la cantidad / se debe hacer una salida / debe validar contra el inventario
  ...
  @Override
  public List<ProductosXClienteComext> consultarProductosActivosXCliente(Long idCliente, List<Long> productosAExcluir) {
    LOGGER.trace("Metodo: <<consultarProductosActivosXCliente>>");
    if (idCliente == null) {
      throw new IllegalArgumentException();
    }
    StringBuilder query = new StringBuilder("SELECT p FROM ProductosXClienteComext p JOIN FETCH p.productosInventario pi JOIN FETCH pi.productosInventarioComext WHERE p.pk.idCliente= :cliente AND p.activo = true ");
