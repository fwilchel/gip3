...
public class ComercioExteriorEJB implements ComercioExteriorEJBLocal {
	...
	public void modificarFP(Documento fp, List<ProductosXDocumento> productosSeleccionados, LogAuditoria auditoria) throws Exception {
		...
		// try {
		// logAuditoria = logAuditoriaDAO.buscarRegistroPorConsultaJPQL(jpql,
		// parametros);
		// } catch (EJBTransactionRolledbackException ex) {
		// throw new
		// Exception("No se encontro en el log de auditoria el estado anterior de la FP.",
		// ex);
		logAuditoria = new LogAuditoria();
		logAuditoria.setValorAnterior(new Long(ConstantesDocumento.ACTIVO).toString());
		// }
		if (logAuditoria != null) {
			Long estadoAnterior = new Long(logAuditoria.getValorAnterior().trim());
			// validacion temporal
			if (estadoAnterior == new Long(ConstantesDocumento.ASIGNADA)) {
