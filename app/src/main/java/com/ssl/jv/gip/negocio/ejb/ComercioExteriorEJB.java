...
public class ComercioExteriorEJB implements ComercioExteriorEJBLocal {
  ...
  public void modificarFP(Documento fp, List<ProductosXDocumento> productosSeleccionados, LogAuditoria auditoria) throws Exception {
    ...
    sql.append("INNER JOIN usuarios ON log_auditoria.id_usuario=usuarios.id ");
    sql.append("INNER JOIN funcionalidades ON log_auditoria.id_funcionalidad=funcionalidades.id ");
    sql.append("WHERE log_auditoria.id_funcionalidad = ? ");
    sql.append("AND log_auditoria.id_reg_tabla = ? ");
    sql.append("ORDER BY log_auditoria.fecha DESC ");
    sql.append("LIMIT 1");
    String valorAnterior = documentoDAO.buscarRegistroPorConsultaNativa(sql.toString(), 126L, fp.getId());
    if (valorAnterior == null) {
      throw new Exception("No se encontro en el log de auditoria el estado anterior de la FP.");
    } else {
      Long estadoAnterior = new Long(valorAnterior.trim());
      // validacion temporal
      if (Objects.equals(estadoAnterior, (long) ConstantesDocumento.ASIGNADA)) {
        estadoAnterior = new Long(ConstantesDocumento.ACTIVO);
      }
      // actualizar el documento
      ...
      try {
        ...
        // actualizar el detalle
        List<ProductosXDocumento> productosPreseleccionados = this.consultarProductosDocumentoComercioExterior(fp.getId());
        this.actualizarProductosDocumentoComercioExterior(fp, productosSeleccionados, productosPreseleccionados, auditoria);
      } catch (Exception ex) {
        LOGGER.error("Error en <<record>> ->> mensaje ->> {} / causa ->> {} ");
        throw new Exception("No es posible actualizar la FP.", ex);
      }
    }
  }
  ...
  private void actualizarProductosDocumentoComercioExterior(Documento doc, List<ProductosXDocumento> productosSeleccionados, List<ProductosXDocumento> productosPreseleccionados, LogAuditoria auditoria) throws Exception {
  ...
  public List<ProductosXClienteComext> consultarProductosActivosXCliente(Map<String, Object> filtro) {
    ...
    StringBuilder query = new StringBuilder("SELECT p FROM ProductosXClienteComext p JOIN FETCH p.productosInventario pi JOIN FETCH pi.productosInventarioComext pic JOIN FETCH pic.tipoLoteoic WHERE p.pk.idCliente= :cliente AND p.activo = true ");
    if (solicitudCafe) {
  ...
  public List<ProductosXDocumento> consultarProductosSP(Long sp, Long cliente) {
    ...
    List<ProductosXDocumento> lista;
    {
      ...
      lista = productoXDocumentoDAO.buscarPorConsultaNombrada(ProductosXDocumento.BUSCAR_PRODUCTOS_DOCUMENTO_COMERCIO_EXTERIOR, parametros);
    }
    {
      // TODO: esto seria innecesario, si cuando se ingresa la SP, se guardan
      // bien los valores, sobre todo el precio y la cantidad.
  ...
  @Override
  public List<ProductosXDocumento> consultarProductosDocumentoComercioExterior(Long sp) {
    LOGGER.trace("Metodo: <<consultarProductosDocumentoComercioExterior>>");
    if (sp == null) {
      throw new IllegalArgumentException();
    }
    Map<String, Object> parametros;
  ...
  @Override
  @TransactionAttribute(TransactionAttributeType.REQUIRED)
  public void anularDocumentoComercioExterior(Documento documento, LogAuditoria auditoria) throws Exception {
    LOGGER.trace("Metodo: <<consultarProductosFP>>");
    auditoria.setValorAnterior(documento.getEstadosxdocumento().getId().getIdEstado().toString());
    documento.getEstadosxdocumento().getId().setIdEstado((long) ConstantesDocumento.ANULADO);
    documentoDAO.update(documento);
    // ingresar el registro de auditoria
    auditoria.setTabla("Documentos");
    auditoria.setAccion("MOD");
    auditoria.setCampo("Estado");
    auditoria.setValorNuevo(documento.getEstadosxdocumento().getId().getIdEstado().toString());
    auditoria.setFecha(new Timestamp(System.currentTimeMillis()));
    auditoria.setIdRegTabla(documento.getId());
    this.logAuditoriaDAO.add(auditoria);
    Map<Long, BigDecimal> saldos = this.consultarUltimosSaldos();
    List<ProductosXDocumento> productosPreseleccionados = this.consultarProductosDocumentoComercioExterior(documento.getId());
    for (ProductosXDocumento pxdPreseleccionado : productosPreseleccionados) {
      if (pxdPreseleccionado.getProductosInventario().getProductosInventarioComext().getControlStock()) {
        BigDecimal ultimoSaldo = saldos.get(pxdPreseleccionado.getProductosInventario().getProductosInventarioComext().getIdProducto());
        if (ultimoSaldo == null) {
        ...
        MovimientosInventarioComext movimientoInventario = new MovimientosInventarioComext();
        movimientoInventario.setConsecutivoDocumento(documento.getConsecutivoDocumento());
        movimientoInventario.setCantidad(pxdPreseleccionado.getCantidad1());
        movimientoInventario.setFecha(new Timestamp(System.currentTimeMillis()));
        movimientoInventario.setProductosInventarioComext(pxdPreseleccionado.getProductosInventario().getProductosInventarioComext());
        movimientoInventario.setSaldo(ultimoSaldo.add(pxdPreseleccionado.getCantidad1()));
