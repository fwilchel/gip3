<html>
  <head>
    <title>
      Lista de Reportes para Compilar
    </title>
  </head>
  <body bgcolor="#ffffff">
    <h1>
      Lista de Reportes para Compilar
    </h1>
    <ol>
      <li><a href="compilarReporte.jsp?reporte=Test.jrxml">Test</a></li>
      <li><a href="compilarReporte.jsp?reporte=report_CintaMagnetica.jrxml">report_CintaMagnetica.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=report_CintaMagneticaCE.jrxml">report_CintaMagneticaCE.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=report_ComprobanteInfDiario.jrxml">report_ComprobanteInfDiario.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=report_ComprobanteInfDiarioCE.jrxml">report_ComprobanteInfDiarioCE.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_FN.jrxml">Report_FN.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_FP.jrxml">Report_FP.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_FX.jrxml">Report_FX.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_FXORIGINAL.jrxml">Report_FXORIGINAL.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_IE.jrxml">Report_IE.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_ITL.jrxml">Report_ITL.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_LE_ANT.jrxml">Report_LE_ANT.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_LE.jrxml">Report_LE.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_OD.jrxml">Report_OD.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_VD.jrxml">Report_VD.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_PCE.jrxml">Report_PCE.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=report_Remision.jrxml">report_Remision.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_SP.jrxml">Report_SP.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_SP2.jrxml">Report_SP2.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_VCE.jrxml">Report_VCE.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=report2_FE.jrxml">report2_FE.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=FacturaDirecta.jrxml">FacturaDirecta.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=report2FDO.jrxml">report2FDO.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=report2ORIGINAL.jrxml">report2ORIGINAL.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=ReporteOrdenDespachoVF.jrxml">ReporteOrdenDespachoVF.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=DevolucionTiendaBodega.jrxml">DevolucionTiendaBodega.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=ItemsCL.jrxml">ITemsCL.jrxml</a></li>
      <li><a href="compilarReporte.jsp?reporte=Report_CL.jrxml">Report_CL.jrxml</a></li>
    </ol>
  </body>
</html>
